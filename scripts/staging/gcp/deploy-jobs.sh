...
UMBRELLA_GCP_PROJECT="${UMBRELLA_GCP_PROJECT:-${GCP_PROJECT}}"
UMBRELLA_GCP_REGION="${UMBRELLA_GCP_REGION:-${GCP_REGION}}"
...
DEPLOY_ARGS=(
  ...
  --set-env-vars="NODE_ENV=production,UMBRELLA_ARTIFACT_STORAGE_MODE=${ARTIFACT_MODE},UMBRELLA_ARTIFACT_LOCAL_DIR=${ARTIFACT_LOCAL_DIR},UMBRELLA_DATA_DIR=${ARTIFACT_LOCAL_DIR},UMBRELLA_GCS_ARTIFACT_BUCKET=${ARTIFACT_BUCKET},UMBRELLA_GCS_ARTIFACT_PREFIX=${ARTIFACT_PREFIX},RUNNER_SCHEDULE=${RUNNER_SCHEDULE},UMBRELLA_GCP_PROJECT=${UMBRELLA_GCP_PROJECT},UMBRELLA_GCP_REGION=${UMBRELLA_GCP_REGION}"
)

if [[ -n "${JOBS_SA_EMAIL}" ]]; then
  DEPLOY_ARGS+=(--service-account="${JOBS_SA_EMAIL}")
fi
