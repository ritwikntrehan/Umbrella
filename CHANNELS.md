...
1. `top_line` (required)
2. `what_changed` (required)
3. `why_it_matters` (required)
4. `data_snapshot` (optional)
5. `watchlist_1_4_weeks` (optional)
6. `custom_work_cta` (required)

Required section keys for `published` status:
- `top_line`
- `what_changed`
- `why_it_matters`
- `custom_work_cta`

Optional empty-state behavior:
- If `data_snapshot` has no qualifying metrics, include a one-line placeholder: "No deterministic metric deltas this cycle."
- If `watchlist_1_4_weeks` has no qualified items, include a one-line placeholder: "No near-term watchlist additions this cycle."
...
- Editorial outputs are bulletin-ready when generated by Stage 7.
...
- Stage 7: channel editorial template and generation policy.
- Stage 8: channel bulletin + optional umbrella highlight synthesis.
