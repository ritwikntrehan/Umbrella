...
import { MockManufacturingAdapter } from "@umbrella/source-adapters";
...
const baseSource = manufacturingSources[0];

if (!baseSource) {
  throw new Error("Missing manufacturing source fixture.");
}

test("mock manufacturing adapter fetch is deterministic for base fixture", async () => {
  const adapter = new MockManufacturingAdapter();
...
test("manufacturing normalization emits expected record shape", async () => {
  const adapter = new MockManufacturingAdapter();
  const rawAssets = await adapter.fetch(baseSource);
  const normalized = await runNormalization(baseSource, rawAssets);

  assert.equal(normalized.length, 1);
  assert.equal(normalized[0]?.externalId, "MFG-CAP-100");
  assert.equal(normalized[0]?.payload.sourceId, baseSource.id);
  assert.equal(normalized[0]?.payload.signalClass, "capability-change");
});

test("manufacturing change detection reports no_change for stable input and changed for updated fixture", async () => {
  const adapter = new MockManufacturingAdapter();
  ...
  assert.deepEqual(changed.addedExternalIds, ["MFG-OPS-240"]);
